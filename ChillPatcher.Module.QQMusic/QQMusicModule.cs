...
namespace ChillPatcher.Module.QQMusic
{
    ...
    public class QQMusicModule : IMusicModule, IStreamingMusicSourceProvider, ICoverProvider, IFavoriteExcludeHandler
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // Create PCM stream
            var streamId = _bridge.CreatePcmStream(songInfo.Mid, bridgeQuality, songInfo.Duration);
            if (streamId < 0)
            {
                _logger?.LogError($"Failed to create PCM stream: {_bridge.GetLastError()}");
                return null;
            }

            // Create reader
            var reader = new QQMusicPcmStreamReader(
                _bridge,
                streamId,
                44100,
                2,
            ...
            var ready = await Task.Run(() => reader.WaitForReady(timeoutMs), cancellationToken);

            if (!ready)
            {
                _logger?.LogWarning($"Stream not ready after {timeoutMs}ms for {songInfo.Name}");
                reader.Dispose();
                return null;
            }
